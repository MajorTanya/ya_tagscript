...
class BreakBlock(Block):
    """
    The Break Block will force the tag output to only be the payload of this block, if
    the passed expression evaluates true.
    ...
    and returns its message while the break block continues to process blocks. If
    command blocks exist after the break block, they will still execute.
